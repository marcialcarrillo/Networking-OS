...
char *Mailbox::receive(int type) //flag 0 = package contratista
{
	mesg_buffer recv;
...
void Mailbox::sendName(const char *name, int id, long totalBytes)
{
	mesg_name package;
	package.mtype = 4;
...
string Mailbox::receiveName()
{
	...
	copy = copy + "$" + to_string(recv.totalBytes) + "-" + to_string(recv.id);
	return copy;
}
